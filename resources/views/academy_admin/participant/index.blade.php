        ...
        <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    ...
                    <a href="{{ route('academy_admin.participant.index') }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Semua Pegawai</a>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6 overflow-x-auto text-gray-900 dark:text-gray-100">
                    ...
                    <table id="detail_table" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @forelse ($participant as $index => $value)
                    ...
                    <nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                        <span class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">Menampilkan <span class="font-semibold text-gray-900 dark:text-white">1-10</span> dari <span class="font-semibold text-gray-900 dark:text-white">{{ $participant_sum }}</span></span>
                        <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                            ...
                            @if ($current_page <= 5)
                                @for ($i = 1; $i <= 5; $i++)
                                    ...
                                    @endif
                                @endfor
                            @elseif ($current_page > 5 && $current_page < ($participant_page_numbers-5))
                                <li>
                                    <a href="#" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">...</a>
                                </li>
                            ...
                            @elseif ($current_page >= ($participant_page_numbers-4) && $current_page <= ($participant_page_numbers))
                                <li>
                                    <a href="#" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">...</a>
                                </li>
                                @for ($i = ($participant_page_numbers-4); $i <= $participant_page_numbers; $i++)
                                    @if ($current_page == $i)
                                        <li>
                                            <a href="#" aria-current="page" class="flex items-center justify-center px-3 h-8 text-blue-600 border border-gray-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{$i}}</a>
                                        </li>
                                ...
                                @endfor
                            @endif
                            @if ($current_page < $participant_page_numbers)
                                <li>
        ...
        .then((response) => {
            if (response.isConfirmed) {
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "{{route('academy_admin.participant.delete')}}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "id": participant_id
                    },
                        ...
                        if(response != 'failed to delete'){
                            ...
                            .then((feedback)=>{
                                if(feedback.isConfirmed){
                                    window.location = "{{ route('academy_admin.participant.index') }}";
                                }
                            })
                        }else{
                            Swal.fire({
        ...
        .then((response) => {
            if (response.isConfirmed) {
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "{{ route('academy_admin.participant.recover') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "id": participant_id
                    },
