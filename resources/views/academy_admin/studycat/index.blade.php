                        ...
                        <tbody>
                            @forelse ($studycat as $index => $item)
                                ...
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap {{$row_color}}">{{ $item->study_material_category }}</td>
                                    ...
                                    <td class="whitespace-nowrap {{$row_color}}">
                                        <div class="md:flex flex-row items-center gap-x-3">
                                            @if ($item->is_active == 1)
                                                <x-edit-button class="mx-auto" href="{{ route('academy_admin.studycat.edit', $item->id) }}">
                                                    Ubah
                                                </x-edit-button>
        ...
        .then((response) => {
            if (response.isConfirmed) {
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "{{ route('academy_admin.studycat.recover') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "id": studycat_id
                    },
