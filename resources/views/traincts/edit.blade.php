    ...
    <div class="p-4 sm:ml-64">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <form method="post" action="{{ route('traincts.update', $item->id) }}" id="edit_traincts" class="mt-6 space-y-6">
                    @csrf
                    {{-- @method('put') --}}

                    <div>
                        <x-input-label for="tc_name" :value="__('Nama Training Center')" />
                        <x-text-input id="tc_name" name="tc_name" type="text" class="mt-1 block w-full" value="{{ $item->tc_name }}"/>
                        @error('tc_name')
                    ...
                    <div>
                        ...
                        <x-text-input id="tc_address" name="tc_address" type="text" class="mt-1 block w-full" value="{{ $item->tc_address }}"/>
                        @error('tc_address')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <x-textarea-input id="tc_note" name="tc_note" class="mt-1 block w-full">{{ $item->note }}</x-textarea-input>
                        @error('tc_note')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
