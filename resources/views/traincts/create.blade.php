    ...
    <div class="p-4 sm:ml-64">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <form method="POST" action="{{ route('traincts.store') }}" class="mt-6 space-y-6">
                    @csrf
                    {{-- @method('put') --}}

                    <div>
                        <x-input-label for="tc_name" :value="__('Nama Training Center')" />
                        <x-text-input id="tc_name" name="tc_name" type="text" class="mt-1 block w-full" value="{{ old('tc_name') }}"/>
                        @error('tc_name')
