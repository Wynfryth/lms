        ...
        <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    ...
                    <a href="{{ route('trainers') }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Master Instruktur</a>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-3">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6 overflow-x-auto text-gray-900 dark:text-gray-100">
                    <h6 class="text-lg font-semibold leading-tight text-center">Master Instruktur</h6>
                    <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between mb-4">
                        ...
                        <x-add-button href="{{route('trainers.create')}}">
                            + Tambah
                        </x-add-button>
                        @endcan
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="trainers_kywd" id="table-search" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Cari data" value="{{ $trainers_kywd }}">
                        </div>
                    </div>
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table id="detail_table" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                @forelse ($trainers as $index => $value)
                                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $index + $trainers->firstItem() }}
                                        </th>
                                        <td class="px-6 py-4">
                                            {{ $value->Employee_name }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            @php
                                                $sesi_kelas = explode(',', $value->sesi_kelas)
                                            @endphp
                                            @forelse ($sesi_kelas as $sesi)
                                                <li>{{$sesi}}</li>
                                            @empty
                                        ...
                                        <td class="px-6 py-4" width="15%">
                                            @if ($value->is_active == 1)
                                                <div class="flex flex-column sm:flex-row flex-wrap space-y-2 sm:space-y-0 items-center justify-between">
                                                    ...
                                                    <a type="button" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" href="{{ route('trainers.edit', $value->id) }}">Edit</a>
                                                    @endcan
...
<script>
                        ...
                        if(response != 'failed to delete'){
                            ...
                            .then((feedback)=>{
                                if(feedback.isConfirmed){
                                    window.location = "{{ route('trainers') }}";
                                }
                            })
                        }else{
                            Swal.fire({
    ...
    $('body').off('keypress', '[name="trainers_kywd"]').on('keypress', '[name="trainers_kywd"]', function(e){
        if(e.which == 13) {
            var trainers_kywd = $(this).val();
            var url = "{{ route('trainers', ['trainers_kywd' => ':trainers_kywd']) }}";
            url = url.replace(':trainers_kywd', trainers_kywd);
