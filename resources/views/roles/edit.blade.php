    ...
    <div class="py-3">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <form method="post" action="{{ route('roles.update', $role_id) }}" class="mt-6 space-y-6">
                    @csrf
                    {{-- @method('put') --}}
                    ...
                    <div class="grid lg:grid-cols-2 sm:grid-cols-1 gap-4">
                        <div class="my-1">
                            ...
                            <x-text-input id="role_name" name="role_name" type="text" class="mt-1 block w-full serialize" value="{{ $role_data->name }}"/>
                            @error('role_name')
                    ...
                    <div class="grid lg:grid-cols-5 sm:grid-cols-1 gap-4 px-3">
                        @php
                            $header = '';
                            $granted_permissions = explode(',', $role_data->granted_permissions);
                        @endphp
                        @foreach ($permissions as $permission)
                            ...
                            @endif
                            @php
                                if(in_array($permission->id, $granted_permissions)){
                            ...
                            <div class="my-1">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer serialize" name="perm_{{ $permission->id }}" {{ $check_status }}>
                                    <div class="relative w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
